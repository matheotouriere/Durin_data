...
tms_info2$start_date = tms_info$end_date %m-% months(2)
tms_info12$start_date = tms_info$end_date %m-% months(12)

# Start and end are list 
#start <-  as.POSIXct(as.Date(active_metadata$date_in, format = "%d/%m/%Y"), tz = "UTC")
start_t2 <- tms_info2$start_date
start_t12 <- tms_info12$start_date
# we choose this date to have the same data for all TMS loggers

localities = as.vector(names(tms$localities))

tms2 <- mc_prep_crop(tms, start = start_t2, end = NULL, localities)
tms12 <- mc_prep_crop(tms, start = start_t12, end = NULL, localities)


tms.all.calc2 <- mc_calc_vwc(tms2, soiltype = "universal") #reseach on soiltype and TOMST VERSION
temp_env_all2  <-tms2 |> 
  mc_env_temp(period = "day", min_coverage = 0.1) |> 
  filter(value >= -30 & value <= 50)
temp_env_all2$locality_id = as.numeric(temp_env_all2$locality_id)

moist_env_all2 <- mc_env_moist(tms.all.calc2, period = "day", min_coverage = 0.1)
moist_env_all2$locality_id = as.numeric(moist_env_all2$locality_id)

tms.all.calc12 <- mc_calc_vwc(tms12, soiltype = "universal") #reseach on soiltype and TOMST VERSION
temp_env_all12  <-tms12 |> 
  mc_env_temp(period = "day", min_coverage = 0.1) |> 
  filter(value >= -30 & value <= 50)
temp_env_all12$locality_id = as.numeric(temp_env_all12$locality_id)

moist_env_all12 <- mc_env_moist(tms.all.calc12, period = "day", min_coverage = 0.1)
moist_env_all12$locality_id = as.numeric(moist_env_all12$locality_id)

#2 month value 

VWC.soil_0_15_cm.mean2 <- moist_env_all2 |> 
...
VWC.soil_0_15_cm.sd2 <- moist_env_all2 |> 
...
T.soil_8_cm.mean2 <- temp_env_all2 |> 
...
T.soil_8_cm.drange2 <- temp_env_all2 |> 
...
T.soil_8_cm.frostdays2 <- temp_env_all2 |> 
...
T.air_2_cm.mean2 <- temp_env_all2 |> 
...
T.air_2_cm.drange2 <- temp_env_all2 |> 
...
T.air_2_cm.frostdays2 <- temp_env_all2 |> 
...
T.air_15_cm.mean2 <- temp_env_all2 |> 
...
T.air_15_cm.drange2 <- temp_env_all2 |> 
...
VWC.soil_0_15_cm.mean12 <- moist_env_all12 |> 
  filter(sensor_name == "VWC.soil_0_15_cm.mean") |> 
  group_by(locality_id) |> 
  summarise(VWC.soil_0_15_cm.mean = mean(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
VWC.soil_0_15_cm.sd12 <- moist_env_all12 |> 
  filter(sensor_name == "VWC.soil_0_15_cm.sd") |> 
  group_by(locality_id) |> 
  summarise(VWC.soil_0_15_cm.sd = mean(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
T.soil_8_cm.mean12 <- temp_env_all12 |> 
  filter(sensor_name == "T.soil_8_cm.mean") |> 
  group_by(locality_id) |> 
  summarise(T.soil_8_cm.mean = mean(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
T.soil_8_cm.drange12 <- temp_env_all12 |> 
  filter(sensor_name == "T.soil_8_cm.drange") |>
  group_by(locality_id) |> 
  summarise(T.soil_8_cm.drange = mean(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
T.soil_8_cm.frostdays12 <- temp_env_all12 |> 
  filter(sensor_name == "T.soil_8_cm.frostdays") |> 
  group_by(locality_id) |> 
  summarise(T.soil_8_cm.frostdays = sum(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
T.air_2_cm.mean12 <- temp_env_all12 |> 
  filter(sensor_name == "T.air_2_cm.mean") |>
  group_by(locality_id) |> 
  summarise(T.air_2_cm.mean = mean(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
T.air_2_cm.drange12 <- temp_env_all12 |> 
  filter(sensor_name == "T.air_2_cm.drange") |>
  group_by(locality_id) |> 
  summarise(T.air_2_cm.drange = mean(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
T.air_2_cm.frostdays12 <- temp_env_all12 |> 
  filter(sensor_name == "T.air_2_cm.frostdays") |> 
  group_by(locality_id) |> 
  summarise(T.air_2_cm.frostdays = sum(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
T.air_15_cm.mean12 <- temp_env_all12 |> 
  filter(sensor_name == "T.air_15_cm.mean") |> 
  group_by(locality_id) |> 
  summarise(T.air_15_cm.mean = mean(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
T.air_15_cm.drange12 <- temp_env_all12 |> 
  filter(sensor_name == "T.air_15_cm.drange") |> 
  group_by(locality_id) |> 
  summarise(T.air_15_cm.drange = mean(value, na.rm = TRUE)) |> 
  ungroup() |> 
...
```{r}
rm(list = setdiff(ls(), c("PV_data", "SP_data", "cond_data","leaf_trait_data", "TOMST_data", "TOMST_data2", "TOMST_data12")))
```
...
# Loading annotations 
annotations <- fromJSON("D:/Utilisateurs/Touriere/Documents/GEN/4A/SIRD/Data/Stomatal_peels/DURIN_SP_area_relabel.v2i.coco/_annotations.coco.json")
...
plant_id_data2 <- plant_id_data |> 
  ...
  left_join(select(TOMST_data2, -siteID, -habitat), by = join_by(plant_id)) |>
  left_join(select(SP_data,
            plant_id, species, stomatal_conductance_mean_p, stomatal_conductance_min, stomatal_conductance_max),
            by = join_by(plant_id))
  

plant_id_data_OF2 <-filter(plant_id_data2, habitat %in% c("Open", "Forested")) |> 
   mutate(siteID = ifelse(siteID == "Senje", "Senja", siteID))
plant_id_data_OF$siteID <- factor(plant_id_data_OF$siteID, levels = c("Senja", "Kautokeino", "Lygra","Sogndal"))

plant_id_data12 <- plant_id_data |> 
  select(plant_id, siteID, habitat, mean_stomatal_density, sd_stomatal_density, mean_stomatal_area_um2, "sd_stomatal_area_um2" = sd_area_um2, mean_leaf_area, sd_leaf_area, mean_leaf_thickness, sd_leaf_thickness,
                   mean_leaf_wetmass, sd_leaf_wetmass, plant_mean_height) |> 
  left_join(select(TOMST_data12, -siteID, -habitat), by = join_by(plant_id)) |>
  left_join(select(SP_data,
            plant_id, species, stomatal_conductance_mean_p, stomatal_conductance_min, stomatal_conductance_max),
            by = join_by(plant_id))
  

plant_id_data_OF12 <-filter(plant_id_data12, habitat %in% c("Open", "Forested")) |> 
   mutate(siteID = ifelse(siteID == "Senje", "Senja", siteID))
plant_id_data_OF$siteID <- factor(plant_id_data_OF$siteID, levels = c("Senja", "Kautokeino", "Lygra","Sogndal"))

plant_id_data_DN <-filter(plant_id_data2, !habitat %in% c("Open", "Forested"))
plant_id_data_LY_O<- plant_id_data2 |> 
  filter(siteID == "Lygra") |> 
  filter(habitat != "Forested")

SP_data_picture_OF <-filter(SP_data_picture, habitat %in% c("Open", "Forested"))|> 
   mutate(siteID = ifelse(siteID == "Senje", "Senja", siteID))
SP_data_picture_OF$siteID <- factor(SP_data_picture_OF$siteID, levels = c("Senja", "Kautokeino", "Lygra","Sogndal"))
...
fig_save <- "D:/Utilisateurs/Touriere/Documents/GEN/4A/SIRD/Rapport/Images/R/new"
...
f_grid_stomatal_tradeoff <- ggplot(SP_data_picture_OF, aes(x = mean_stomatal_area_um2_p, y = stomatal_density_p, color = species)) +
  ...
  stat_poly_eq(aes(label = paste(..eq.label.., ..adj.rr.label.., ..p.value.label.., sep = "~~~")), 
               formula = y ~ x, 
               parse = TRUE, 
               coef.digits = 2,  # Limite le nombre de chiffres significatifs pour les coefficients
               rr.digits = 2,    # Limite le nombre de chiffres significatifs pour R²
               p.digits = 2,
               size = 2)+   # Limite le nombre de chiffres significatifs pour la p-value
...
# Ajuster et résumer le modèle linéaire pour chaque espèce
model_summaries <- SP_data_picture_OF %>%
  group_by(species) %>%
  summarize(
    model = list(lm(stomatal_density_p ~ mean_stomatal_area_um2_p, data = cur_data()))
  ) %>%
  mutate(summary = lapply(model, summary))

# Afficher species, siteID, et le résumé du modèle
for (i in 1:nrow(model_summaries)) {
  cat("Species:", model_summaries$species[i], "\n")
...
for (i in 1:nrow(model_summaries)) {
  cat("Species:", model_summaries$species[i], "\n")
  cat("SiteID:", as.character(model_summaries$siteID[i]), "\n")
  print(model_summaries$summary[[i]])
  cat("\n-----------------\n")
}
...
tradeoff_join <- ggarrange(
  f_grid_stomatal_tradeoff2, f_grid_stomatal_tradeoff,
  labels = c("a)", "b)"),
  ncol = 1,
...
ggsave(filename = paste0(fig_save, "/tradeoff_join.png"), plot = tradeoff_join, width = 10, height = 12, units = "in", dpi = 300)
```
...
leaf_join <- ggarrange(
  f_grid_plant_height_tradeoff,f_grid_leaf_area_tradeoff,
  labels = c("a)", "b)"),
  ncol = 1,
...
wet_join <- ggarrange(
  f_grid_leaf_thickness_tradeoff,f_grid_leaf_wetmass_tradeoff,
  labels = c("a)", "b)"),
  ncol = 1,
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_density ~ plant_mean_height, data = plant_id_data_OF_VM)
resume_modeleVM_height<- summary(modeleVM)
print(resume_modeleVM_height)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_density ~ plant_mean_height, data = plant_id_data_OF_VV)
resume_modeleVV_height<- summary(modeleVV)
print(resume_modeleVV_height)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_density ~ mean_leaf_area, data = plant_id_data_OF_VM)
resume_modeleVM_leafarea<- summary(modeleVM)
print(resume_modeleVM_leafarea)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_density ~ mean_leaf_area, data = plant_id_data_OF_VV)
resume_modeleVV_leafarea<- summary(modeleVV)
print(resume_modeleVV_leafarea)

#coefficient of linear regression
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_density ~ mean_leaf_thickness, data = plant_id_data_OF_VM)
resume_modeleVM_leaftk<- summary(modeleVM)
print(resume_modeleVM_leaftk)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_density ~ mean_leaf_thickness, data = plant_id_data_OF_VV)
resume_modeleVV_leaftk<- summary(modeleVV)
print(resume_modeleVV_leaftk)

#coefficient of linear regression
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_density ~ mean_leaf_wetmass, data = plant_id_data_OF_VM)
resume_modeleVM_leafwm<- summary(modeleVM)
print(resume_modeleVM_leafwm)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_density ~ mean_leaf_wetmass, data = plant_id_data_OF_VV)
resume_modeleVV_leafwm<- summary(modeleVV)
print(resume_modeleVV_leafwm)
...
ggsave(filename = paste0(fig_save, "/wetmass_join.png"), plot = wet_join, width = 10, height = 12, units = "in", dpi = 300)
```
...
f_grid_plant_height_tradeoff2 <- plant_id_data_OF |> 
 ...
 # geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.2, position = position_dodge(width = 0.75)) + # Ajoute des barres d'erreur
 # facet_grid(. ~ siteID) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "Plant height (cm)") +
   theme(legend.position = "bottom")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
...
f_grid_leaf_area_tradeoff2 <- plant_id_data_OF |> 
 ...
 # geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.2, position = position_dodge(width = 0.75)) + # Ajoute des barres d'erreur
 # facet_grid(. ~ siteID) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "Plant leaf area (cm²)") +
   theme(legend.position = "bottom")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
...
f_grid_leaf_thickness_tradeoff2 <- plant_id_data_OF |> 
 ...
 # geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.2, position = position_dodge(width = 0.75)) + # Ajoute des barres d'erreur
 # facet_grid(. ~ siteID) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "plant leaf thickness (mm)") +
   theme(legend.position = "bottom")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
...
f_grid_leaf_wetmass_tradeoff2 <- plant_id_data_OF |> 
 ...
 # geom_errorbar(aes(ymin = ymin, ymax = ymax), width = 0.2, position = position_dodge(width = 0.75)) + # Ajoute des barres d'erreur
 # facet_grid(. ~ siteID) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "plant leaf wetmass (g)") +
   theme(legend.position = "bottom")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_area_um2 ~ plant_mean_height, data = plant_id_data_OF_VM)
resume_modeleVM_height<- summary(modeleVM)
print(resume_modeleVM_height)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_area_um2 ~ plant_mean_height, data = plant_id_data_OF_VV)
resume_modeleVV_height<- summary(modeleVV)
print(resume_modeleVV_height)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_area_um2 ~ mean_leaf_area, data = plant_id_data_OF_VM)
resume_modeleVM_leafarea<- summary(modeleVM)
print(resume_modeleVM_leafarea)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_area_um2 ~ mean_leaf_area, data = plant_id_data_OF_VV)
resume_modeleVV_leafarea<- summary(modeleVV)
print(resume_modeleVV_leafarea)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_area_um2 ~ mean_leaf_thickness, data = plant_id_data_OF_VM)
resume_modeleVM_leaftk<- summary(modeleVM)
print(resume_modeleVM_leaftk)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_area_um2 ~ mean_leaf_thickness, data = plant_id_data_OF_VV)
resume_modeleVV_leaftk<- summary(modeleVV)
print(resume_modeleVV_leaftk)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_area_um2 ~ mean_leaf_wetmass, data = plant_id_data_OF_VM)
resume_modeleVM_leafwm<- summary(modeleVM)
print(resume_modeleVM_leafwm)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_area_um2 ~ mean_leaf_wetmass, data = plant_id_data_OF_VV)
resume_modeleVV_leafwm<- summary(modeleVV)
print(resume_modeleVV_leafwm)
...
```{r}
# Créer le graphique avec les points et les barres d'erreur
f_grid_plant_soil_vwc2 <- plant_id_data_OF2 |> 
  filter(!is.na(habitat)) |>
...
f_grid_plant_soil_vwc12 <- plant_id_data_OF12 |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_density, x = VWC.soil_0_15_cm.mean, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
...
# Afficher le graphique
print(f_grid_plant_soil_vwc12)

f_grid_plant_soil_T2 <- plant_id_data_OF2 |> 
  filter(!is.na(habitat)) |>
...
f_grid_plant_soil_T12 <- plant_id_data_OF12 |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_density, x = T.soil_8_cm.mean, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
...
# Afficher le graphique
print(f_grid_plant_soil_T12)

f_grid_plant_soil_Frost2 <- plant_id_data_OF2 |> 
  filter(!is.na(habitat)) |>
...
f_grid_plant_air_T2cm2 <- plant_id_data_OF2 |> 
  filter(!is.na(habitat)) |>
...
f_grid_plant_air_T2cm12 <- plant_id_data_OF12 |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_density, x = T.air_2_cm.mean, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
...
# Afficher le graphique
print(f_grid_plant_air_T2cm12)

f_grid_plant_air_FD2cm2 <- plant_id_data_OF2 |> 
  filter(!is.na(habitat)) |>
...
f_grid_plant_air_FD2cm12 <- plant_id_data_OF12 |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_density, x = T.air_2_cm.frostdays, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
...
# Afficher le graphique
print(f_grid_plant_air_FD2cm12)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_density ~ VWC.soil_0_15_cm.mean, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_density ~ VWC.soil_0_15_cm.mean, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_density ~ T.soil_8_cm.mean, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_density ~ T.soil_8_cm.mean, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_density ~ T.air_2_cm.mean, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_density ~ T.air_2_cm.mean, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_density ~ T.air_2_cm.frostdays, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_density ~ T.air_2_cm.frostdays, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)
...
f_grid_plant_soil_vwc2 <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_area_um2, x = VWC.soil_0_15_cm.mean, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
#  facet_grid(. ~ species) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "Volumetric water content")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
    axis.text.x = element_text(size = 12), 
...
f_grid_plant_soil_T2 <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_area_um2, x = T.soil_8_cm.mean, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
#  facet_grid(. ~ species) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "Soil Temperature (°C)")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
    axis.text.x = element_text(size = 12), 
...
f_grid_plant_soil_Frost2 <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_area_um2, x = T.soil_8_cm.frostdays, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
#  facet_grid(. ~ species) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "Number of frostdays")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
    axis.text.x = element_text(size = 12), 
...
f_grid_plant_air_T2cm2 <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_area_um2, x = T.air_2_cm.mean, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
#  facet_grid(. ~ species) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "2cm Air Temperature (°C)")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
    axis.text.x = element_text(size = 12), 
...
f_grid_plant_air_FD2cm2 <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_area_um2, x = T.air_2_cm.frostdays, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
#  facet_grid(. ~ species) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "Number of frostdays")+ 
...
f_grid_plant_air_T15cm2 <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |>
  ggplot(aes(y = mean_stomatal_area_um2, x = T.air_15_cm.mean, color = species)) +
  geom_point(aes(shape = siteID), size = 3) + # Ajoute des points avec des formes différentes pour chaque espèce
  geom_smooth(aes(color = species), method = "lm") + # Ajoute des lignes de régression séparées pour chaque espèce
#  facet_grid(. ~ species) + # Crée des facettes pour chaque site
  labs(y = "Stomatal area (µm²)", x = "15cm Air Temperature (°C)")+ 
  theme(
    axis.title.x = element_text(size = 14), 
    axis.title.y = element_text(size = 14), 
    axis.text.x = element_text(size = 12), 
...
sa_join <- ggarrange(
  f_grid_plant_soil_vwc2, f_grid_plant_soil_T2,
  labels = c("a)", "b)"),
  ncol = 1,
  nrow = 2,
  common.legend = TRUE,
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_area_um2 ~ VWC.soil_0_15_cm.mean, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_area_um2 ~ VWC.soil_0_15_cm.mean, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_area_um2 ~ T.soil_8_cm.mean, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_area_um2 ~ T.soil_8_cm.mean, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_area_um2 ~ T.air_2_cm.mean, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_area_um2 ~ T.air_2_cm.mean, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(mean_stomatal_area_um2 ~ T.air_2_cm.frostdays, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(mean_stomatal_area_um2 ~ T.air_2_cm.frostdays, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)

ggsave(filename = paste0(fig_save, "/Sa_Tsoil2.png"), plot = f_grid_plant_soil_T2, width = 10, height = 6, units = "in", dpi = 300)
ggsave(filename = paste0(fig_save, "/Sa_join.png"), plot = sa_join, width = 10, height = 12, units = "in", dpi = 300)
...
plant_id_data_OF_VM <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VM")
modeleVM <-  lm(sd_stomatal_density ~ VWC.soil_0_15_cm.sd, data = plant_id_data_OF_VM)
resume_modeleVM<- summary(modeleVM)
print(resume_modeleVM)
plant_id_data_OF_VV <- plant_id_data_OF |> 
  filter(!is.na(habitat)) |> 
  filter(species == "VV")
modeleVV <-  lm(sd_stomatal_density ~ VWC.soil_0_15_cm.sd, data = plant_id_data_OF_VV)
resume_modeleVV<- summary(modeleVV)
print(resume_modeleVV)
```
...
species <- "VM"
  ...
  cat("**Espèce :**", species, "/n")
  cat("**Densité stomatique :**/n")
  shapiro.test(residuals(model1))
  cat("**Aire stomatique moyenne :**/n")
  shapiro.test(residuals(model2))

  # Post-hoc (si ANOVA significative)
  cat("**Résultats de l'ANOVA :**/n")
  print(summary(model1))
  print(summary(model2))
  if(summary(model1)[[1]]$Pr[1] < 0.05) {
    cat("Comparaison multiple pour la densité stomatique : /n")
    print(TukeyHSD(model1))
  }
  if(summary(model2)[[1]]$Pr[1] < 0.05) {
    cat("Comparaison multiple pour l'aire stomatique moyenne : /n")
    print(TukeyHSD(model2))
  }

  # Corrélations
  cat("**Corrélations clim :**/n")

...
start_t <- as.POSIXct(as.Date("01/04/2023", format = "%d/%m/%Y"), tz = "UTC")
...
tms.cleaning.soil <- tms |> 
  mc_filter(sensors = "TMS_moist") |> 
  mc_reshape_long()
  
tms.cleaning.temp.air <- tms |> 
  mc_filter(sensors = c("TMS_moist","TMS_T1"), reverse = TRUE) |> 
    mc_reshape_long() |> 
  filter(value <= -30 | value >= 50) |> 
  select(serial_number, sensor_name, value) |> 
  group_by(serial_number) |> 
  summarise(number = n())
  
tms.cleaning.temp.soil <- tms |> 
  mc_filter(sensors = "TMS_T1") |> 
    mc_reshape_long() |> 
...
## summary data.frame of snow estimation
tms.snow <- mc_calc_snow_agg(tms.all.calc)
 
temp_env_all  <-tms |> 
  mc_env_temp(period = "week", min_coverage = 0.9) |> 
  filter(value >= -30 & value <= 50)

moist_env_all <- mc_env_moist(tms.all.calc, period = "day", min_coverage = 0.9)
...
f_line_macro_1 <-ggplot(moist.mean.all, aes(x = datetime, y = mean.moist, color = site)) +
  ...
  labs(#title = "Average Soil Moisture Over Time",
       x = "Date",
       y = "Mean Soil Volumetric Water Content",
       color = "Site")
...
print(f_line_macro_2)
 ...
 f_line_micro_1 <-ggplot(moist.mean.all.hab, aes(x = datetime, y = mean.moist.hab, color = habitat)) +
  ...
  labs(#title = "Average Soil Moisture Over Time",
       x = "Date",
       y = "Mean Soil Solumetric Water Sontent",
       color = "Habitat")+
...
f_line_micro_3 <-ggplot(snow.mean.all, aes(x = datetime, y = snow.hab, color = habitat)) +
  geom_line() +
  facet_wrap(~ site) +
  labs(#title = "Average air temperature Over Time",
       x = "Date",
...
FDD0.mean.all <- temp_env_all |> 
  filter(sensor_name == "T.air_2_cm.FDD0") |> 
